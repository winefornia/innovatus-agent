...
def test_dollar_price_single_item_captured_as_manual_price():
    ...
    assert ext["items"][0]["regular_unit_price_cents"] == 7500
...
def test_decimal_and_thousands_separator():
    ...
    assert ext["items"][0]["regular_unit_price_cents"] == 125000
...
def test_four_digit_year_is_vintage_not_price():
    ...
    assert "regular_unit_price_cents" not in ext["items"][0]
...
def test_multi_item_price_not_auto_assigned():
    ...
    _apply_clarification_facts("$75", ext)
    assert "regular_unit_price_cents" not in ext["items"][0]
    assert "regular_unit_price_cents" not in ext["items"][1]


def test_existing_price_not_overwritten():
    ext = _items({"product_name": "Viognier", "regular_unit_price_cents": 6000})
    _apply_clarification_facts("$75", ext)
    assert ext["items"][0]["regular_unit_price_cents"] == 6000
